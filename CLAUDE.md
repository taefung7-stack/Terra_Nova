...
- **`textbook/`** — 월간 구독 교재 생산 파이프라인 (Schema v2.2, build-fullbook).
  ...
  - **문제·구문·단어장 품질 규칙 (모든 고1·2·3 지문, 2026-06-26~ / validator 강제)**:
    ① **정답 길이 균형** — 객관식 5개 보기를 비슷한 길이로(정답이 혼자 길면 학생이 길이로
    정답을 찍음). 정답 간결화 우선. validator WARN(정답 최장+오답평균 1.4배↑).
    ② **본문↔문제 밑줄 정합** — "밑줄 친 …" stem이 인용하는 영어 구는 본문 표현과 **정확히
    일치** + `page1.body`에 `<u>…</u>` 실제 밑줄 + evidence 동기화. validator **ERROR(차단)**.
    ③ **단어장 고급어** — `page4.vocab`은 본문 실제 등장 + 학년 이상 난이도(중학 기초어 금지,
    본문에서 가장 어려운 단어 선별). validator WARN(본문 미등장).
    ④ **한글 띄어쓰기** — 모든 한글 필드 표준 띄어쓰기. validator WARN(이중공백·구두점앞공백 등).
    ⑤ **구문 인라인 해석** — page3 해석은 `translation_ko`의 `[n]` 마커로 쪼개 각 영어 문장
    아래 한 줄로 렌더(하단 통합블록 폐지). `[n]` 개수=문장 수 필수. 규칙: `templates/AUTHORING-GUIDE.md`.
