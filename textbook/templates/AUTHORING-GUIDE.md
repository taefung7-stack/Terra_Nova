...
- **정답 길이 균형 (필수)**: 정답 보기가 오답들보다 눈에 띄게 길면 학생이 "가장 긴 게
  정답"이라고 눈치챈다. **5개 보기를 비슷한 길이 밴드**로 맞춰라. 정답을 간결화하는
  방향이 우선(오답을 억지로 늘리지 말 것). validator가 정답이 최장+오답 평균 1.4배↑면
  WARN. (단일 단어 빈칸형은 정답 단어가 본질적으로 길 수 있어 예외)
- **밑줄 정합 (필수, 차단)**: stem이 "밑줄 친 …"으로 본문 표현을 인용하면 (주로 함의
  추론), **그 영어 구가 본문 표현과 정확히 일치**하고 `page1.body`에 `<u>…</u>`로
  실제 밑줄이 있어야 한다. 인용구는 본문 실제 표현 기준으로 맞춘다(예: stem "left
  deliberate gaps" ✗ → 본문 "left empty gaps on purpose"로 통일 + `<u>` 감쌈).
  `answers.explanations[].evidence`의 인용도 본문과 동기화. validator가 불일치 시 ERROR.
...
- `sentences`의 index와 정확히 일치해야 합니다 **(중요)**: 렌더러가 `[n]` 마커로
  해석을 쪼개 **각 영어 문장 바로 아래 한 줄**로 인라인 배치한다(2026-06-26~).
  하단 통합 해석 블록은 없어졌다. `[n]` 개수 ≠ 문장 수면 일부 문장 해석이 누락되니,
  **모든 문장에 대응하는 `[n] …` 조각이 빠짐없이** 있어야 한다. 빌드 시 `console.warn`로
  누락 경고.
...
- **난이도 (필수)**: 단어장은 **본문(page1.body)에 실제 등장**하면서 **해당 학년 이상
  난이도**인 단어로 구성한다. `element`·`column`·`bond`·`predict` 같은 중학 기초어는
  금지 — 본문에서 **가장 어렵거나 학술적·저빈도인 단어**(예: vigorously, inert,
  deliberate)를 골라라. 단어장은 "본문에서 가장 어려운 단어 모음"이어야 한다.
  validator가 본문 미등장 단어에 WARN. 학년별 톤: 고1<고2<고3 순으로 난이도 상향.
