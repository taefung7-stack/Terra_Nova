#!/usr/bin/env node
/**
 * 2026-08 고등부(Saturn 고1 / Jupiter 고2 / Sun 고3) fullbook PDF 압축.
 *
 * 왜 필요한가:
 *   고등 8월호는 와이드 배너 삽화(16:5, 180mm 전폭)가 무손실(FlateDecode)로 다수 들어가
 *   합본이 176~197MB대 → Supabase 프로젝트 전역 업로드 제한(~50MB) 초과.
 *
 * 해결:
 ...
 *
 * 파이프라인 위치: build-fullbook → **이 압축** → _finalize-2026-08-highschool → protect/upload
 *   (finalize 를 먼저 돌리면 140p 완성본이 압축 전 크기로 굳으니 순서 주의.)
 *
 * 입력/출력: dist/2026-08/2026-08-{Level} (학년)/2026-08-{Level}-fullbook.pdf  (in-place 교체)
 *   - 압축본은 임시파일로 만든 뒤, 페이지수(134p)·크기(<50MB) 검증 후 원본 교체.
 *
 * 사용: node tools/_compress-2026-08-highschool.mjs                 (전부)
 *        node tools/_compress-2026-08-highschool.mjs --levels Sun    (특정 레벨만)
 *        node tools/_compress-2026-08-highschool.mjs --dpi 150       (DPI 조정)
 */
...
const { values } = parseArgs({
  options: { dpi: { type: 'string', default: '170' }, levels: { type: 'string' } }
});
const DPI = Number(values.dpi);
const GS = process.env.GS_BIN || 'gswin64c';

const want = values.levels
  ? values.levels.split(',').map(s => s.trim().toLowerCase()).filter(Boolean)
  : null;
const LEVELS = want ? ALL_LEVELS.filter(l => want.includes(l.level.toLowerCase())) : ALL_LEVELS;
...
console.log(`=== 2026-08 고등부 fullbook 압축 (DPI=${DPI}, 목표 <${LIMIT_MB}MB) ===\n`);
let allOk = true;
...
console.log(`\n${allOk ? '완료. <50MB 압축 + 134p 보존.' : '일부 실패 — 위 메시지 확인(DPI 낮춰 재시도 가능).'}`);
