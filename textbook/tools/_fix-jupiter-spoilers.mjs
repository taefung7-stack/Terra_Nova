// One-shot fix: strip blank-fill answer spoilers from 2026-06-J (Jupiter) passages.
...
const dir = resolve(here, '..', 'content', 'passages', '2026-06-J');
...
for (const f of files) {
  ...
  // English: page3.sentences[*].segments[*].text — strip "  (= ...)" or "  (...)" after "____"
  for (const sent of data.page3?.sentences ?? []) {
    for (const seg of sent.segments ?? []) {
      if (typeof seg.text === 'string' && /_+\s*\(/.test(seg.text)) {
        ...
        // Match underscores followed by "(...)" with optional "= " prefix
