...
> 유닛: **U1**(완료) · **U2**(원문 전사 대기 — 아래 「Unit 2」 절 참조)
...
**상태: 원문 전사 대기.** 파이프라인·검증기·저작 스펙은 배선 완료, `data/U2/*.json`
저작은 정본이 채워진 뒤 시작한다.
...
- `passage` 는 U1 과 마찬가지로 **본문 + 블로그를 이어붙인 하나의 배열**이다.
  블로그를 빠뜨리면 검증이 차단한다.
- p.24 상단 도입 문단(4문장)은 **Ch1 맨 앞**에 넣는다.
...
`_SOURCE-U2.js` 의 각 챕터 `sentences` / `blog.sentences` 배열이 **비어 있다.**
교과서 실물을 보고 채운 뒤 아래 검증기를 돌린다.
