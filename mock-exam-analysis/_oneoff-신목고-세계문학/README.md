...
Ch7 원문은 38개 배열 원소(약 4,027자)로 다른 챕터(최대 816자)보다 훨씬 길다.
...
원문은 대화·서술이 섞인 49개 문법 문장이지만, `passage` 배열 자체를 38개로 자연스럽게
묶었다(짧은 대사 "No." 등은 앞뒤와 합침). 분석 카드는 31개로, 문장 다수를 한 카드에
묶은 경우 `covers` 에 하위 번호를 순서대로 나열했다. 검증은 `covers` 숫자 자체가 아니라
**카드 en_html 을 전부 이어붙인 문자열이 passage 배열을 이어붙인 문자열과 일치**하는지로
확인했다(verify.mjs 4번 항목과 동일 원리, Ch7 은 수동 스크립트로 확인). em dash 앞뒤
공백 하나 차이로 불일치가 난 적이 있다 — 새 카드 작성 시 대시·공백까지 원문과 정확히
맞출 것.
