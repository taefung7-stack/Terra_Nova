...
> 유닛: **U1**(완료) · **U2**(완료 — 삽화 4장만 미반영, 아래 「Unit 2」 절 참조)
...
**상태: 3종 제작 완료 (2026-09-08).** 정본 전사 → 분석지 → 워크북 → 변형문제까지
모두 끝났고 검증기 5종이 전부 오류 0 · 경고 0 이다.
**남은 것은 삽화 4장뿐** — 프롬프트는 각 `data/U2/{N}.json` 의 `illustration.prompt`
에 이미 저작돼 있고, 이미지를 만들어 `dist/U2/assets/illust-{1..4}.jpg` 로 넣은 뒤
재렌더·재합본하면 된다(U1 「삽화」 절 절차 그대로).
