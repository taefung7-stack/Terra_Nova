...
> · **U3**(완료 — 삽화 미반영, 아래 「Unit 3」 절 참조 — Ch1~6)
> · **U3 Ch7**(본문분석·변형문제만 완료, 워크북 없음 — 아래 「Ch7」 절 참조)
...
## Ch7 — The Hundred-Foot Journey (추가 지문, 교과서 pp.68~72)

**상태: 본문분석 + 변형문제만 완료(2026-09-21). 워크북 없음(사용자 요청).**

Unit 3 안의 "Flavors of the World" 소재지만, 기존 Ch1~6(면 요리 소개, pp.58~63)과는
**별개의 소설 발췌문**이다(리처드 C. 모라이스, *The Hundred-Foot Journey*). Mallory 부인이
사고로 입원한 Hassan을 찾아가 페이스트리로 화해하고, 그 사고가 결국 우정과 문화적 융합으로
이어진다는 내용.

| 산출물 | 페이지 | 파일 |
|--------|--------|------|
| 본문분석 (개별) | 24p | `dist/U3/7.pdf` |
| 변형문제 (개별) | 통합 variant-book 안에 포함 | `dist/U3/variant-book.pdf` (72p 중) |

### ⚠️ `_SOURCE-U3.js` 에 등록하지 않았다 — 의도적

- Ch1~6 은 "면 요리" 정본(`_SOURCE-U3.js`)에서 파생되지만, Ch7 은 완전히 다른 지문이라
  **정본 파일에 추가하지 않았다.** `verify.mjs`/`verify-source.mjs` 는 Ch7 을 검사하지 않는다.
  대신 `data/U3/7.json` 자체 내부 정합성(카드 en_html 이어붙이면 passage 전체와 일치)을
  수동 스크립트로 확인했다 — `verify-tags.mjs` 는 전 데이터 폴더를 스캔하므로 Ch7 도 포함된다.
- `combine.mjs` 는 `dist/{N}.html` 을 **파일명으로 스캔**해서 합본에 넣으므로 Ch7 도
  **자동으로 본문분석 합본(61p)에 포함된다.** 다만 목차(ToC) 제목은 `_SOURCE-U3.js`
  조회 실패로 **"Chapter 7"** 일반명으로 나온다(사용자 확인, 그대로 두기로 함 — 본문
  페이지 자체 제목은 정상).
- `combine-variant.mjs` 는 `variant-book.html` 전체를 그대로 쓰므로 **변형문제 합본에도
  Ch7 이 자동 포함**된다(72p, 6챕터 대비 +15p).
- 워크북은 만들지 않았으므로 `workbook-combined.pdf` 는 여전히 6챕터 60p 그대로다.

### 지문이 5~8배 길다 — 서술형 공유 지문 잘림 주의

Ch7 원문은 37개 배열 원소(약 4,025자)로 다른 챕터(최대 816자)보다 훨씬 길다.
`build-variant.mjs --shared-writing-passage` 는 `data.passage`(원문 전체)를
`_orig_passage` 로 주입해 서술형(본문 제시) 공유 페이지 상단에 한 번에 보여주는데,
이 지문 길이로는 **A4 한 장 예산을 넘어 182px 잘림**이 발생했다.
→ `7-variant.json` 최상위에 **`writing_passage`**(축약 발췌, 8문장)를 지정해 해결.
`origPassage = v.writing_passage || v._orig_passage || …` 우선순위를 이용한 것으로,
공유 빌더(`build-variant.mjs`)는 전혀 수정하지 않았다. 새 지문이 유난히 길 때 재사용할 패턴.

### 카드 병합 — 49문장을 30카드로

원문은 대화·서술이 섞인 49개 문법 문장이지만, `passage` 배열 자체를 37개로 자연스럽게
묶었다(짧은 대사 "No." 등은 앞뒤와 합침). 분석 카드는 30개로, 문장 다수를 한 카드에
묶은 경우 `covers` 에 하위 번호를 순서대로 나열했다. **`covers` 숫자는 passage 배열
인덱스가 아니라 저작 당시 49문장 초안 기준 번호**이므로 배열 인덱스와 1:1로 대응하지
않는다(참고용, verify 대상 아님). 검증은 그 대신 **카드 en_html 을 전부 이어붙인
문자열이 passage 배열을 이어붙인 문자열과 일치**하는지로 확인한다
(verify.mjs 4번 항목과 동일 원리, Ch7 은 수동 스크립트로 확인).

### 2026-09-21 재검수에서 발견·수정한 결함 — 따옴표 경계 오류

원본과 재대조하다가 p.71 "Well, there's also a thin layer …" 문장이 실제로는
**따옴표 하나로 이어지는 한 발화**(Hassan이 재료를 쭉 나열)인데, `passage` 배열에서
`…honey."` 로 닫고 `"And you've…` 로 다시 여는 **존재하지 않는 인용부호 경계**를
만들어 두 문장으로 조용히 쪼갰던 것을 발견했다. `passage`/`passage_ko` 를 다시 병합하고
해당 분석 카드 2장(구 no.20·21)도 하나로 합쳐 30카드로 줄였다. em dash 앞뒤 공백 하나
차이로도 불일치가 난 적이 있다 — 새 카드 작성 시 대시·공백·따옴표 경계까지 원문과
정확히 맞출 것. 재검수 방법: `passage` 배열을 이어붙인 문자열을 이미지에서 독립적으로
재전사한 텍스트와 문자 단위로 diff — 카드↔passage 자체 정합만으로는 이 결함을 못 잡는다
(둘 다 똑같이 틀렸기 때문).
