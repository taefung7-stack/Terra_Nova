 ...
 *
 * 사용법:
 *   node _oneoff-신목고-세계문학/verify.mjs        # 전체
 *   node _oneoff-신목고-세계문학/verify.mjs U1     # 특정 유닛만
 * =================================================================== */
...
/** 정본 → 검증용 평탄화.
 *  이 교과서는 게시글(body) 아래에 댓글(comments)이 붙는 구조라, 원문 정본은 둘을
 *  나눠 보관한다. 분석지 JSON 의 passage 는 '게시글 문장 + 댓글 문장'을 순서대로
 *  이어붙인 하나의 배열이므로, 검증 시에도 같은 순서로 펼쳐 대조한다.
 *  (댓글을 빠뜨리는 누락 사고를 막기 위해 댓글도 원문 대조 대상에 포함한다) */
function flatten(src) {
  return src.map(ch => ({
    ...ch,
    sentences: [
      ...ch.sentences,
      ...ch.comments.flatMap(c => c.sentences),
    ],
  }));
}
