  ...
  for (const ch of SOURCE) {
    ...
    for (const g of groups) {
      ...
      list.forEach((s, i) => {
        ...
        // 문장 끝 구두점 — 닫는 인용부호(곧은/곡선)와 이모티콘 :) 도 허용한다.
        // U2 는 대화문이 많아 곡선 따옴표(”’)로 닫히는 문장이 정상이고,
        // 블로그 글에는 ':)' 로 끝나는 문장이 실제로 있다.
        if (!/(?:[.!?][")'”’\]]*|[.!?]\s*[:;]-?[)(])$/.test(t)) {
          warn(`${at} 끝에 문장부호가 없음 (잘림 의심): "…${t.slice(-42)}"`);
        }

        // 약어에서 끊긴 흔적 — 조각이 약어로 끝나면 다음 조각과 붙어야 할 가능성.
        // 단 p.m./a.m. 은 문장 맨 끝에 오는 것이 정상이므로(“…got home at 5 p.m.”)
        // 뒤에 본문이 이어져야 하는 호칭·서수 약어만 본다.
        if (/\b(St|Mr|Mrs|Ms|Dr|Prof|etc|vs|approx)\.$/i.test(t)) {
