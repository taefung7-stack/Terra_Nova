#!/usr/bin/env node
/* ===================================================================
 * 워크북 데이터 무결성 검증 — 신서고 YBM 영어II (L1·L2)
 * ===================================================================
 *
 ...
 *
 * 사용법:
 *   node _oneoff-신서고-YBM-L1/verify-workbook.mjs        # L1·L2 전부
 *   node _oneoff-신서고-YBM-L1/verify-workbook.mjs L1
 * =================================================================== */
...
function verifyOne(lesson, n) {
  ...
  const dataPath = path.join(HERE, 'data', 'EX2', `${n}.json`);
  const wbPath   = path.join(HERE, 'data', 'EX2', `${n}-workbook.json`);
  ...
  const passage = ((data.passage_corrected && data.passage_corrected.length === (data.passage || []).length)
    ? data.passage_corrected : (data.passage || [])).map(norm);
...
for (const lesson of lessons) {
  console.log(`── ${lesson} ──`);
  for (let n = 1; n <= 4; n++) verifyOne(lesson, n);
  console.log('');
}
