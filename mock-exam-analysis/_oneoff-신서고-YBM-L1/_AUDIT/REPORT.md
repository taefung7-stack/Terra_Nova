...
### ③ 2차 점검 — 정답 "위치" 외의 찍기 단서 (2026-08-26 추가)

분포를 맞춘 뒤, 번호 분포만으로는 잡히지 않는 단서 3종을 추가 점검했다.

| 결함 | 상태 | 조치 |
|---|---|---|
| **정답 길이 단서** — 내용형 48문항 중 최장 보기가 정답인 경우 32건(67%, 기대 20%) | **수정 전부터 존재**(내 순열 재배치가 만든 것 아님) | 2위와 5자 이상 벌어진 11건 **정답만 간결화**(의미 불변, 오답 불변) → 최장=정답 24/48(50%), 5자 이상 격차 **0건** |
| **EX2 무관한문장 ④④④④** | 4지문 전부 같은 자리 | 무관 문장을 2/5/3/4 로 이동. 나머지 문장 **상대 순서 보존**(논리 전개), 해설 번호 동기화 |
| **EX2 어휘 ④⑤⑤⑤** | 오답 낱말이 매번 마지막 문장 | EX2/3·/4 는 마지막 낱말을 정상화하고 다른 밑줄을 오용으로 교체 → ④⑤③② |

**파생 결함 — ①~⑤ 뒤 조사 불일치 16건**: 번호를 재배치하면 숫자는 바뀌지만
뒤따르는 조사는 그대로 남아 `"③가 무관하다"`(→③이), `"②이 적절하다"`(→②가)
가 된다. 읽기 기준 **①일·③삼 = 받침 O → 이/은/과**, **②이·④사·⑤오 = 받침 X → 가/는/와**.
`_fix-particles.mjs` 로 전건 교정. **정답 번호를 손대는 스크립트는 반드시 조사 검사를 동반할 것.**
...
| ⑤ | 8 (9.1%) | 16 | **15 (17.0%)** |
| 최장 연속 동일정답 | 16 | 4 | **2** |
| 최장보기=정답 | 67% | 65% | **50%** (5자↑ 격차 0건) |
| 유형별 3연속 동일정답 | — | 2건 | **0건** |

검증: verify 6종 전부 오류 0 · 경고 0 / overflow 0 / 페이지 수 불변(38p·37p) /
원문 60문장 verbatim 유지 / 해설 번호 정합 88/88 / 조사 오류 0.
