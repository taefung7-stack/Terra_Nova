...
const LESSONS = {
  L6: {
    source: SOURCE_L6,
    lessonNo: 6,
    coverTitle: '목일중 3학년<br>동아 영어 3',
    titleEn: 'Make the World Beautiful',
    coverSub: '동아(이병민) 중학교 영어 3<br>Lesson 6 · Make the World Beautiful',
    docTitle: '목일중 3학년 · 동아 Lesson 6 본문분석 합본 — Terra Nova',
    out: '목일중3_동아이병민_Lesson6_본문분석_합본.pdf',
  },
  L7: {
    source: SOURCE_L7,
    lessonNo: 7,
    coverTitle: '목일중 3학년<br>동아 영어 3',
    titleEn: 'Feel the Wonder',
    coverSub: '동아(이병민) 중학교 영어 3<br>Lesson 7 · Feel the Wonder',
    docTitle: '목일중 3학년 · 동아 Lesson 7 본문분석 합본 — Terra Nova',
    out: '목일중3_동아이병민_Lesson7_본문분석_합본.pdf',
  },
  L8: {
    source: SOURCE_L8,
    lessonNo: 8,
    coverTitle: '목일중 3학년<br>동아 영어 3',
    titleEn: 'Up to You',
    coverSub: '동아(이병민) 중학교 영어 3<br>Lesson 8 · Up to You',
...
for (const f of files) {
  ...
  /* 2026-09-01: 새 미드저니 사진을 받아 illust-{2,3,4}.png 가 '합성본 → 순수 사진'
     으로 교체됐다. 상단 삽화를 제거하던 임시 블록은 더 이상 필요 없어 삭제.
     (INTRO = 미드저니 사진 / PASSAGE flow 아래 = 교과서 도해 로 슬롯이 분리됨) */

  /* ★ 2026-09-01: 교과서 도해는 이제 build.mjs 가 data/L6/*.json 의
     textbook_figure 필드를 읽어 직접 렌더한다(PASSAGE 면 LOGIC FLOW 아래).
     여기서 또 끼워 넣으면 같은 도해가 두 번 나오므로 주입하지 않는다.
     LESSONS[].figures 는 캡션 참고용으로만 남겨 둔다. */
  const fig = null;
  if (fig) {
    let placed = false;
...
const cover = `<section class="page cover-page">
  <div class="cover-wrap">
    ...
    <div class="cover-title">${LESSON.coverTitle ?? '신서고 3학년 2학기<br>중간고사 대비'}</div>
...
const extraCss = `
  /* 교과서 도해 — LOGIC FLOW 아래.
     본문 폭 전면(100%)은 너무 커서 페이지를 잡아먹는다 → 68% 로 줄이고 가운데 정렬.
     (사용자 요청 2026-09-01) */
  .tb-figure { margin: 9px 0 0; text-align: center; }
  .tb-figure img {
    width: 68%; max-width: 68%; height: auto; display: block; margin: 0 auto;
