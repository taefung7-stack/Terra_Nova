...
> 인쇄된다. `.fulltext` 기본값(`gap:4px`, `line-height:1.45`)으로는 31문장이 1375px 가 되어
> 본문 영역(1017px)을 넘겼다. `combine.mjs` 의 `.fulltext-all` 규칙으로 gap 제거 +
> 행간·폰트를 줄여 맞췄다(L6 최종 993px/1017px). **문장 수가 더 많은 과를 추가하면
> 반드시 puppeteer 실측으로 재확인할 것** — 눈으로는 잘린 걸 못 알아챈다.
