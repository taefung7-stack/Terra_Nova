#!/usr/bin/env node
/* ===================================================================
 * 워크북 데이터 무결성 검증 — 신서중2 미래엔(문영인) (L1·L2)
 * ===================================================================
 *
 ...
 *
 * 사용법:
 *   node _oneoff-신서중2-미래엔/verify-workbook.mjs        # L5·L6 전부
 *   node _oneoff-신서중2-미래엔/verify-workbook.mjs L1
 * =================================================================== */
...
const lessons = arg ? [arg.toUpperCase()] : ['L5', 'L6'];
// 과마다 챕터 수가 다르다(L5 3개 / L6 4개) — 정본 길이를 그대로 따른다.
const chapterCount = (L) => fs.readdirSync(path.join(HERE, 'data', L))
  .filter(f => /^\d+\.json$/.test(f)).length;

console.log('🔍 워크북 무결성 검증\n');
for (const lesson of lessons) {
  console.log(`── ${lesson} ──`);
  for (let n = 1; n <= chapterCount(lesson); n++) verifyOne(lesson, n);
  console.log('');
}
