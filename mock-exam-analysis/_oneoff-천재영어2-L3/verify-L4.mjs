#!/usr/bin/env node
/* ===================================================================
 * 천재(조수경) 영어II Lesson 4 — 무결성 검증 (문장 누락 0 보장)
 * ===================================================================
 ...
 *
 * 사용법:
 *   node _oneoff-천재영어2-L3/verify.mjs
 * =================================================================== */
...
const LESSONS = [
  { id: 'L4', label: 'Lesson 4 · Flavors Without Borders', source: SOURCE },
];
...
// 이 폴더는 과가 하나뿐이라 data 가 평면 구조다(신서고는 data/{L1,L2}).
const DATA_DIR = path.join(__dirname, 'data', 'L4');
...
for (const ch of SOURCE) {
  ...
  const IRREGULAR = {
    // 이 과(씨앗)의 불규칙 동사 — bind/bound 는 어간 절단으로 잡히지 않는다.
