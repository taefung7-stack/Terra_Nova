#!/usr/bin/env node
/* ===================================================================
 * 워크북 데이터 무결성 검증 — 천재(조수경) 영어II Lesson 4
 * ===================================================================
 *
 ...
 *
 * 사용법:
 *   node _oneoff-천재강상구-L2/verify-workbook.mjs        # L1·L2 전부
 *   node _oneoff-천재강상구-L2/verify-workbook.mjs L1
 * =================================================================== */
...
function verifyOne(lesson, n) {
  ...
  const dataPath = path.join(HERE, 'data', 'L4', `${n}.json`);
  const wbPath   = path.join(HERE, 'data', 'L4', `${n}-workbook.json`);
...
// 이 폴더는 과가 하나뿐이라 data 가 평면 구조다.
const lessons = ['L4'];

console.log('🔍 워크북 무결성 검증\n');
for (const lesson of lessons) {
  console.log(`── ${lesson} ──`);
  for (let n = 1; n <= 6; n++) verifyOne(lesson, n);
  console.log('');
}
