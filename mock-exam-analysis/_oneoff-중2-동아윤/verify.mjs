 ...
 *
 * 사용법:
 *   node _oneoff-중2-동아윤/verify.mjs        # L5·L6 전부
 *   node _oneoff-중2-동아윤/verify.mjs L6     # 특정 과만
 * =================================================================== */
...
if (!TARGETS.length) {
  console.error(`알 수 없는 과: ${only} (L5 / L6 / L7 / BY)`);
  process.exit(2);
}
...
for (const ch of SOURCE) {
  ...
  const IRREGULAR = {
    ...
    put: ['put'],
    stop: ['stopped', 'stop'],
    represent: ['represents', 'represent'],
  };
