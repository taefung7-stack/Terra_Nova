...
const LESSONS = {
  L5: {
    ...
    coverSub: '동아(윤정미) 중학교 영어 2<br>Lesson 5 · Street Art in London',
    docTitle: '중2 · 동아(윤정미) Lesson 5 본문분석 합본 — Terra Nova',
    out: '중2_동아윤정미_Lesson5_본문분석_합본.pdf',
  },
  L6: {
    source: SOURCE_L6,
    lessonNo: 6,
    ...
    coverSub: '동아(윤정미) 중학교 영어 2<br>Lesson 6 · Dr. Schofield, a Foreigner Who Loved Korea',
...
<head>
<meta charset="utf-8">
<title>${LESSON.docTitle ?? `중2 · 동아(윤정미) Lesson ${LESSON.lessonNo} 본문분석 합본 — Terra Nova`}</title>
 ...
 * Chrome 이 형광펜(linear-gradient)·box-shadow 를 투명 소프트마스크
 * (/SMask <</S /Luminosity>>)로 뽑는데, 굿노트 PDFKit 은 한 페이지에 이게
 * 수십 개 쌓이면 합성에 실패해 페이지를 통째로 백지로 그린다(에러 없음).
 * ghostscript 로 다시 쓰면 평탄화된다(gs 는 1.5 에서만 평탄화 — 1.4 금지).
 * 겉모습 동일, 용량도 크게 준다(45MB→수MB). 제거 금지. */
